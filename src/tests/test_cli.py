...
def test_to_markupy() -> None:
    ...
    py = """from markupy.tag import Body,H1,Head,Hr,Html,Input,SlButton,Title\nprint(Html[Head[Title["Test"]],Body[H1("#myid.title.header",{"burger&fries":"good"})["Parse me!"],Hr,Input(".my-input",disabled=True,value="0",_click_outside_500ms="test",dataTest="other",dataUrlValid="coucou"),SlButton(hxOn__htmx__configRequest="attri")["Click!"]]])"""
    assert to_markupy(html) == py


def test_escape() -> None:
    html = """<a href="{{ url_for(".index") }}">Hello</a>"""
    py = """from markupy.tag import A\nprint(A(href="{{ url_for(\\".index\\") }}")["Hello"])"""
    assert to_markupy(html) == py
...
def test_jinja() -> None:
    ...
    """
    py = """from markupy.tag import Body,H1,H2,H3,Li,Ol,P\nprint(Body[H1["{{ heading }}"],P["Welcome to our cooking site, {{ user.name }}!"],H2["Recipe of the Day: {{ recipe.name }}"],P["{{ recipe.description }}"],H3["Instructions:"],Ol["{% for step in recipe.steps %}",Li["{{ step }}"],"{% endfor %}"]])"""
    assert to_markupy(html) == py
